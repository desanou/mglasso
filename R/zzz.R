...
install_libs <- function(path) {
    if (!reticulate::py_module_available("scipy")) {
        reticulate::py_install("scipy", method = "auto", conda = "auto",
            pip = TRUE, envname = "mglasso")
    }

    if (!reticulate::py_module_available("scikit-learn")) {
        reticulate::py_install("scikit-learn", method = "auto", conda = "auto",
            pip = TRUE, envname = "mglasso")
    }

    if (!reticulate::py_module_available("six")) {
        reticulate::py_install("six", method = "auto", conda = "auto",
            pip = TRUE, envname = "mglasso")
    }

    if (!reticulate::py_module_available("parsimony.estimators")) {
        # text <- paste('sys.path.insert(0,'', path,
        # '/pylearn-parsimony')', sep = '', collapse = NULL)
...
.onLoad <- function(libname, pkgname) {
    ...
    is_mglasso_env_installed = tryCatch(reticulate::use_miniconda(condaenv = "mglasso",
        required = TRUE), error = function(e) {
        "not installed"
    })
    ...
    if (env_exists && remove_existing_env) {
        msg <- sprintf(paste("Environment \"%s\" already exists.", "Remove the environment..."),
            envname)
        message(msg)
    ...
    # reticulate::source_python('conesta_solver.py') loading
    # conesta solver
...
# git submodule add
# https://github.com/neurospin/pylearn-parsimony.git
# ./inst/python/pylearn-parsimony
# https://github.com/rstudio/tensorflow/blob/701d402efa78fea25c95c1b3394c22b27aabd466/R/install.R
# source
# https://github.com/Azure/azureml-sdk-for-r/blob/master/R/install.R
